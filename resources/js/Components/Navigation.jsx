            ...
            <div className="flex gap-6">
                {tabs.map((tab) => (
                    ...
                    >
                        ...
                        {t(tab.label)}
                        {tab.badge && (
                            <span className="ml-2 px-2 py-0.5 rounded-full bg-(--color-bg-tertiary) text-xs text-(--color-text-secondary)">
                                {tab.badge}
                            </span>
                        )}
                    </button>
...
export function Breadcrumb({ items }) {
    ...
    return (
        <nav
            className="flex items-center gap-2 text-sm text-(--color-text-tertiary) mb-4"
            ...
            {items.map((item, idx) => (
                <div key={idx} className="flex items-center gap-2">
                    ...
                    {item.href ? (
                        ...
                        <span className="text-(--color-text-primary)">{t(item.label)}</span>
                    )}
                </div>
            ))}
        </nav>
            ...
            {items.map((item) => (
                <div key={item.id} className="glass-card overflow-hidden">
                    ...
                    {openItems.includes(item.id) && (
                        <div className="px-4 pb-4 text-(--color-text-tertiary)">
                            {t(item.content)}
                        </div>
                    )}
                </div>
