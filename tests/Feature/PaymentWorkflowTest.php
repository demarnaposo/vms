...
class PaymentWorkflowTest extends TestCase
{
    ...
    public function test_non_compliant_payment_request_alert_localizes_status_label(): void
    {
        ...
        try {
            app(PaymentService::class)->createRequest(
                $this->vendor,
                $this->vendorUser,
                5000,
                'Manual service description'
            );
            $this->fail('Expected a non-compliant payment request to be blocked.');
        } catch (\Exception $exception) {
            $this->assertSame(
                'Vendor tidak patuh (Status: Berisiko). Silakan selesaikan masalah kepatuhan terlebih dahulu.',
                $exception->getMessage()
            );
        }

        $this->assertSame(Vendor::COMPLIANCE_AT_RISK, $this->vendor->refresh()->compliance_status);
    }
